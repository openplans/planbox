...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for project in orm.Project.objects.all():
            for section in project.sections.all():
                section.index += 1
                section.save()
            description_section = orm.Section(
                type='text',
                slug='description',
                index=0,
                details={'content': project.description})
            project.description = ''
            project.sections.add(description_section)
            project.save()
    ...
    def backwards(self, orm):
        ...
        for project in orm.Project.objects.all():
            try:
                description_section = project.sections.get(slug='description', type='text')
            except orm.Section.DoesNotExist:
                continue
            project.description = description_section.details.get('content', '')
            project.save()
            description_section.delete()
    ...
    models = {
        ...
        u'planbox_data.profile': {
            ...
            'project_editor_version': ('django.db.models.fields.PositiveIntegerField', [], {'default': '1'}),
